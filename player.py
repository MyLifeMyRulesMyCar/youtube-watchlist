...
PAGES_ORIGIN = "https://mylifemyrulesmycar.github.io"
PAGES_HTML = os.path.join(BASE_DIR, "docs", "index.html")
...
def generate_pages(videos):
    os.makedirs(os.path.dirname(PAGES_HTML), exist_ok=True)
    with open(PAGES_HTML, "w", encoding="utf-8") as f:
        f.write(build_html(videos, PAGES_ORIGIN))
    return PAGES_HTML
...
def main():
    ...
    parser.add_argument(
        "--pages",
        action="store_true",
        help="Generate docs/index.html for GitHub Pages instead of playing locally",
    )
